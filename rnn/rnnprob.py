...
def main(argv):
	...
	with tf.Session() as sess:
		...
		np_data = np.random.rand(batchsz,sequencesz)
		with tf.name_scope("input") as scope:
			X_ = tf.placeholder(name="X", dtype=tf.float32, shape=(batchsz, sequencesz))
			Y_ = tf.placeholder(name="Y", dtype=tf.float32, shape=(batchsz, 1))

		with tf.variable_scope("rnn") as scope:
			rnn = tf.contrib.rnn.BasicRNNCell(2)
			initial_state = rnn.zero_state(batchsz, tf.float32)
